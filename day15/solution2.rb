...
class Solver
  ...
  def parse_char(char, pos)
    ...
    when 'G' then @monsters.push(Monster.new(pos, char, GOBLIN_POWER))
    when 'E' then @monsters.push(Monster.new(pos, char, ELF_POWER))
    end
  end
...
class Monster
  ...
  def attack(enemies)
    ...
    if hittable
      hittable.damage(@power)
      if hittable.hitpoints <= 0
        if @char == 'G'
          puts 'nope'
          exit
        end
